...
def register_view(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            user = form.save()
            messages.success(request, "Registration successful! Please log in.", user)
...
def employee_register_view(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            user = form.save()
            messages.success(
                request, "Employee registration submitted for approval.", {"user": user}
            )
            return redirect("login")
    else:
...
def claim_booking(request, booking_id):
    if not request.user.profile.is_employee() or not request.user.profile.is_approved:
        messages.e({"success": False, "error": "Access denied"})
        return redirect("dashboard")
...
def complete_booking(request, booking_id):
    ...
    if request.method == "POST":
        ...
        messages.success(
            request,
            f"Job #{booking.ticket_number} submitted for supervisor approval!",
            quality_check,
        )
        return redirect("employee_dashboard")
...
def update_booking_status(request, booking_id):
    ...
    if new_status in ["in_progress", "completed"]:
        if new_status == "completed":
            booking.status = (
                "pending_approval"  # Set to pending approval instead of completed
            )
            booking.work_completed_at = timezone.now()
            messages.success(
                request,
            ...
            if new_status == "in_progress" and not booking.work_started_at:
                ...
                messages.success(
                    request, f"Started work on job #{booking.ticket_number}"
                )

        booking.save()
        return redirect("employee_dashboard")

    return messages.error({"success": False, "error": "Invalid status"})
